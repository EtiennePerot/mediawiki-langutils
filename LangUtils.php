...
class ExtLangUtils {
	...
	public static $sidebarList_NS = array(
		NS_TEMPLATE,
		NS_HELP,
		NS_TALK,
		NS_CATEGORY,
	...
	public static function onParserFirstCallInit( Parser $parser ) {
		...
		$parser->setFunctionHook( 'ifpagelang', [ self::class, 'ifpagelang' ], SFH_OBJECT_ARGS );

		# Variables
		$parser->setFunctionHook( 'pagelang', [ self::class, 'pagelang' ], SFH_NO_HASH );
		$parser->setFunctionHook( 'pagelangsuffix', [ self::class, 'pagelangsuffix' ], SFH_NO_HASH );
		return true;
	}

	public static function MagicWordwgVariableIDs( &$customVariableIds ) {
	...
	public static function getLang( $title, $namespace = false, $returnMatch = false ) {
		...
		if ( $namespace == NS_FILE ) {
			...
			if ( !in_array( $lang, ExtLangUtils::$allowedLanguages ) ) {
				$lang = 'en';
			}
	...
	public static function languagetitle( $parser, $frame, $args ) {
		if ( count( $args ) == 0 || !isset(ExtLangUtils::$coreLanguageNames_titlecase[$args[0]]) ) {
			return '';
		}
		return ExtLangUtils::$coreLanguageNames_titlecase[$args[0]];
	}
	...
	public static function skinAddSidebar( $skin, &$bar ) {
		...
		if ( in_array( $title->getNamespace(), ExtLangUtils::$sidebarList_NS ) ) {
			...
			if ( $namespace == NS_FILE ) {
				...
				if ( count( $tparts ) >= 2 ) {
					if ( $pageLang == 'en' ) {
						$langPrefix = $title->getPrefixedText();
					}
					else {
			...
			foreach ( ExtLangUtils::$allowedLanguages as $lang ) {
				...
				if ( $page->getArticleID() !== 0 ) {
					...
					$localname = isset(ExtLangUtils::$coreLanguageNames_titlecase[$lang]) ? ExtLangUtils::$coreLanguageNames_titlecase[$lang] : $lang;
					$URL = $page->getLinkUrl();
					$output[] = [
						'text'   => $localname,
						'href'   => $URL,
